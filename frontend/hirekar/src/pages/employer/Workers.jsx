...
function Workers() {
  ...
  useEffect(() => {
    const fetchWorkers = async () => {
      try {
        const response = await fetch("http://16.171.195.37/api/worker/all", {
          headers: {
    ...
    const applyFilters = () => {
      ...
      if (filters.designation) {
        filtered = filtered.filter(
          (worker) =>
            worker.designation &&
            worker.designation.toLowerCase() ===
      ...
      if (filters.hourlyRate) {
        ...
        filtered = filtered.filter(
          (worker) => worker.hourly_rate >= min && worker.hourly_rate <= max
        );
      }
