...
const WorkerDetailsPage = () => {
    ...
    useEffect(() => {
        const fetchWorkerDetails = async () => {
            try {
                const response = await fetch(`http://localhost:8000/api/worker/${worker_id}`, {
                    headers: {
    ...
    return (
        <div className="w-full pt-4 pb-12 pr-8">
            <div className="max-w-4xl mx-auto bg-white shadow-md rounded-lg overflow-hidden">
                <div className="">
                    <h2 className="text-2xl font-semibold text-blue-500">{worker.user.name}</h2>
                    <p className="text-gray-500">{worker.user.email}</p>
                    <p className="mt-4 text-sm">Address: {worker.user.address}</p>
                    <p className="mt-2 text-sm">Phone Number: {worker.user.phone_number}</p>
                    ...
                    <p className="mt-2 text-sm mb-6">Average Rating: {calculateAverageRating(worker.reviews)}</p>
                </div>
                <hr />
                        ...
                        {worker.reviews.length > 0 ? (
                            worker.reviews.map((review, index) => (
                                <div key={index} className="p-2 border-b mt-2 ">
                                    <p className="text-gray-600">Rated {review.rating} by {review.employer_id ? review.employer_id.name : 'Unknown'}</p>
